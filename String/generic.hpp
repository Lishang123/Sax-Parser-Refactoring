...
// (potentially cv-qualified) `char*` because we have to guard that one against
// nulls template<typename S>
//     requires(
//     // the ordering is potentially wrong, remove reference -> remove pointer
//     -> remove cv !std::is_same_v<char,
...
// inline std::string_view sv( const S &str ) noexcept { return std::string_view
// { str }; }
...
template <typename S>
  requires(!is_char_pointer_v<S>) &&
          requires(const S &s) { std::string_view{s}; }
inline constexpr std::string_view sv(const S &s) noexcept {
  return std::string_view{s};
}

// A string is anything, that we have a `sv()` function yielding
// `std::string_view` for.
template <typename S>
concept StringTypeLegacy =
    (!std::is_same_v<std::nullptr_t, S>) && requires(S s) {
      { sv(s) } -> std::same_as<std::string_view>;
    };

template <typename S>
concept StringType = (!std::same_as<std::remove_cvref_t<S>, std::nullptr_t>) &&
                     requires(const S &s) {
                       { sv(s) } -> std::same_as<std::string_view>;
                     };

[[nodiscard]]
inline constexpr int compare(const StringType auto &lhs,
                             const StringType auto &rhs) noexcept {
  return sv(lhs).compare(sv(rhs));
}

[[nodiscard]]
inline constexpr int compare(const StringType auto &lhs,
                             const StringType auto &rhs,
 ...
 *		- neither is NULL, and they contain the same values at first
 *Length positions
 *
 * @param lhs		First string to compare
 * @param rhs		Second string to compare
 * @param length	Number of characters to compare.
 *					Only positions 0 to ( Length-1) will be
 *compared.
 *
 * @return Whether first Length letters of Left equals first Length letters of
 *Right
 */

[[nodiscard]]
inline constexpr bool equal(const StringType auto &lhs,
                            const StringType auto &rhs) noexcept {
 ...
 */
[[nodiscard]]
inline constexpr bool empty(const StringType auto &str) noexcept {
  return sv(str).empty();
}
 ...
 */
[[nodiscard]]
inline constexpr size_t length(const StringType auto &str) noexcept {
  return sv(str).length();
}
