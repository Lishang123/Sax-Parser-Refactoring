...
public:
  ...
  std::strong_ordering
  operator<=>(const M::String::StringType auto &rhs) const noexcept {
  ...
  template <typename Functor> decltype(auto) modifyInPlace(Functor &&f) {
    return std::invoke(std::forward<Functor>(f), m_String.get());
  };
};

inline bool operator==(const ST_String &lhs,
                       const M::String::StringType auto &rhs) noexcept {
...
struct KVPairKeyComparator {
  ...
  bool operator()(const std::pair<ST_String, ST_String> &x,
                  const std::pair<ST_String, ST_String> &y) const {
    return x.first < y.first;
  }
};
} // namespace ST

template <> struct std::hash<ST_String> {
...
template <> struct fmt::formatter<ST_String> : formatter<std::string_view> {
