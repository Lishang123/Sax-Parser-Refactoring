...
class XML_Parser : public xercesc::DefaultHandler,
...
private:
  ...
  void m_errorMessage(const xercesc::SAXParseException &E, const char *Code,
                      const char *Description);

protected:
  bool m_UsingXOP; // TODO: never read, only assigned.
  bool m_CurrentCharactersArePlainBinaryData; // TODO: never read, only
                                              // assigned.
   ...
   */
  static const char *m_RemoveXMLDeclaration(const char *Buffer,
                                            size_t BufferSize = -1);

public:
  XML_Parser(bool UseValidation = false, bool UseNamespaces = false,
             bool IgnoreXMLDeclaration = false);
  ...
  virtual bool startElementChar(const char *URI, const char *LocalName,
                                const char *QName,
                                const xercesc::Attributes &CurrentAttributes);

  /** End of the current element. */
  virtual bool endElementChar(const char *URI, const char *LocalName,
                              const char *QName);

  /** Character part of the current element. */
  virtual bool charactersChar(const char *URI, const char *LocalName,
   ...
   * @param message The message describing what happened. It's yours, so delete
   * it when you're done.
   */
  virtual void errorMessage(const M_SystemMessage &message);

  M_SystemMessage
  systemMessageFromException(const xercesc::SAXParseException &E,
                             const char *Code, const char *errorLevel);
  ...
  /** Set parser feature
   * See http://xml.apache.org/xerces-c/program-sax2.html#SAX2Features (dead
   * link)
  ...
  /** Set parser property
   * See http://xml.apache.org/xerces-c/program-sax2.html#SAX2Properties (dead
   * link)
  ...
  xercesc::InputSource *
  resolveEntity(xercesc::XMLResourceIdentifier *resourceIdentifier) override;
   ...
   * @note this function doesn't differentiate non-existing attribute and empty
   * attribute value
   */
  static ST_String getAttributeValue(const xercesc::Attributes &attributes,
                                     std::string_view name);

  static std::optional<long>
  getAttributeLongOptional(const xercesc::Attributes &attributes,
                           std::string_view name);

  static long getAttributeLong(const xercesc::Attributes &attributes,
   ...
   * @pre The value must be lower-case 'true' or 'false', i.e. '0/1' or
   * uppercase values are not supported
   */
  static bool getAttributeBool(const xercesc::Attributes &attributes,
                               std::string_view name, bool &exists);

  static std::optional<bool>
  getAttributeBool(const xercesc::Attributes &attributes,
                   std::string_view name);
};
