...
public:
  ...
  xercesc::BinInputStream *makeStream() const override {
    ...
    return new (getMemoryManager()) xercesc::BinMemInputStream{
        reinterpret_cast<const XMLByte *>(content), size,
        xercesc::BinMemInputStream::BufOpt_Copy, getMemoryManager()};
  }
...
XML_Parser::XML_Parser(bool UseValidation, bool UseNamespaces,
                       bool IgnoreXMLDeclaration)
    : m_UseNamespaces(UseNamespaces),
      m_IgnoreXMLDeclaration(IgnoreXMLDeclaration), m_UsingXOP(false),
      m_Result(false), m_CurrentCharactersArePlainBinaryData(false) {
  XML_xerces_String Validation("http://xml.org/sax/features/validation");
  m_Parser.setFeature(Validation.getXMLForm(), UseValidation);

  if (!UseValidation) {
    // Disable resolving external DTDs, at least if there's no validation at
    // all!! see
    // http://xerces.apache.org/xerces-c/program-sax2-3.html#load-external-dtd
    XML_xerces_String ExternalDTD(
  ...
  XML_xerces_String securityManagerProperty(
      "http://apache.org/xml/properties/security-manager");
  m_Parser.setProperty(securityManagerProperty.getXMLForm(),
                       &m_securityManager);
}
...
void XML_Parser::setOption(const char *Name, const char *Value) {
  ...
  // OptionalValue.m_XMLForm is copied downstream in setExternalSchemaLocation
  // and setExternalNoNamespaceSchemaLocation so no dangling pointer later :)
  m_Parser.setProperty(OptionName.getXMLForm(),
                       const_cast<XMLCh *>(OptionValue.getXMLForm()));
}
...
void XML_Parser::disableEntityResolver() {
  ...
  setOption("http://apache.org/xml/features/disable-default-entity-resolution",
            true);
}

xercesc::InputSource *
XML_Parser::resolveEntity(xercesc::XMLResourceIdentifier *resourceIdentifier) {
...
bool XML_Parser::m_Parse(const char *Buffer, size_t BufferSize) {
  ...
  xercesc::MemBufInputSource MemoryBuffer(
      reinterpret_cast<const unsigned char *>(Buffer), BufferSize, "XML_Buffer",
      false);

  // Avoid additional copying of the input buffer. The requirement is that the
  // buffer remain valid till the end of parsing. Since we parse synchronously
  // and always have the whole buffer at hand, this is true and safe.
  ...
  try {
    ...
    errorMessage(
        M_SystemMessage{"XMLLM_DOMAIN", "XMLLM_XML_PARSER_NATIVE_XML_ERROR",
                        fmt::format("XML error: {}", Error.getLocalForm())});
  } catch (const xercesc::SAXException &E) {
    XML_xerces_String Error(E.getMessage());

    errorMessage(
        M_SystemMessage{"XMLLM_DOMAIN", "XMLLM_XML_PARSER_NATIVE_SAX_ERROR",
...
const char *XML_Parser::m_RemoveXMLDeclaration(const char *Buffer,
                                               size_t BufferSize) {
  ...
  while (*NoWhitespaceBuffer &&
         std::isspace(static_cast<unsigned char>(*NoWhitespaceBuffer))) {
    ++NoWhitespaceBuffer;
  }
  // if there is still enough space in the buffer
  if ((BufferSize -= NoWhitespaceBuffer - Buffer) > 5) {
    if (NoWhitespaceBuffer[0] == '<' && NoWhitespaceBuffer[1] == '?' &&
        NoWhitespaceBuffer[2] == 'x' && NoWhitespaceBuffer[3] == 'm' &&
        NoWhitespaceBuffer[4] == 'l') {
      NoWhitespaceBuffer += 5;
      // scan until "?>" is found
      // legacy code: while( BufferSize >= 2 && NoWhitespaceBuffer[0] != '?' &&
      // NoWhitespaceBuffer[1] != '>')
      while (BufferSize >= 2 &&
             !(NoWhitespaceBuffer[0] == '?' && NoWhitespaceBuffer[1] == '>')) {
        ++NoWhitespaceBuffer;
        --BufferSize;
      }
      if (BufferSize >= 2) {
        NoWhitespaceBuffer +=
            2; // we didn't remove "?>" sequence, so it's high time to do this
      }
    }
  }
...
void XML_Parser::startElement(const XMLCh *const URI, const XMLCh *const Name,
  ...
  ST_String LocalURI;
  LocalURI.consume(m_UseNamespaces ? m_xercesString.convertToLocalForm(URI)
                                   : M::Memory::create(0));
  ST_String LocalName;
  LocalName.consume(m_UseNamespaces
                        ? m_xercesString.convertToLocalForm(Name)
                        // ST_String guarantees NUL termination
                        // duplicate copies to size + 1 and ensures NUL
                        // termination so no need for passing strlen()+1
                        : M::Memory::duplicate(LocalQName.c_str(),
                                               strlen(LocalQName.c_str())));

  endCharacters();

  m_Callstack.emplace_back(std::move(LocalURI), std::move(LocalName),
                           std::move(LocalQName));

  auto &el = m_Callstack.back();
  m_Result = startElementChar(el.uri.c_str(), el.name.c_str(), el.qName.c_str(),
                              ElementAttributes);
}
...
void XML_Parser::endElement(const XMLCh *const URI, const XMLCh *const Name,
  ...
  if (m_Result) {
    m_Result =
        endElementChar(el.uri.c_str(), el.name.c_str(), el.qName.c_str());
  }

  m_Callstack.pop_back();
}

void XML_Parser::characters(const XMLCh *const Characters,
...
void XML_Parser::characters(const std::string &LocalCharacters) {
  ...
  m_CharacterBuffer.writeConsume(
      M::Memory::duplicate(LocalCharacters.c_str(), Length), Length);
}
...
void XML_Parser::endCharacters() {
  ...
  // Only forward call if this character part is part of an element. This
  // filters the call on the start of parsing.
  if (!m_Callstack.empty()) {
...
void XML_Parser::startPrefixMapping(const XMLCh *const Prefix,
...
void XML_Parser::m_errorMessage(const xercesc::SAXParseException &E,
...
void XML_Parser::fatalError(const xercesc::SAXParseException &E) {
  m_errorMessage(E, "XMLLM_XR_REPOSITORYPARSER_NATIVE_FATAL_PARSE_ERROR",
                 "fatal error");
}

void XML_Parser::warning(const xercesc::SAXParseException &E) {
  m_errorMessage(E, "XMLLM_XR_REPOSITORYPARSER_NATIVE_PARSE_WARNING",
                 "warning");
}

bool XML_Parser::startElementChar(
    const char *URI, const char *LocalName, const char *QName,
    const xercesc::Attributes &ElementAttributes) {
  return true;
}

bool XML_Parser::endElementChar(const char *URI, const char *LocalName,
                                const char *QName) {
  return true;
}
...
bool XML_Parser::endPrefixMappingChar(const char *Prefix) { return true; }

bool XML_Parser::charactersChar(const char *URI, const char *LocalName,
                                const char *QName, const char *Chars,
...
M_SystemMessage
XML_Parser::systemMessageFromException(const xercesc::SAXParseException &E,
                                       const char *Code,
  ...
  return M_SystemMessage{
      "XMLLM_DOMAIN", Code,
      fmt::format(
          R"(Native XML parser {} in "{}" line {}, column {}, location {}: "{}")",
          errorLevel, XML_xerces_String{E.getSystemId()}.getLocalForm(),
          static_cast<unsigned long>(E.getLineNumber()),
          static_cast<unsigned long>(E.getColumnNumber()), LocationString,
          XML_xerces_String{E.getMessage()}.getLocalForm())};
}

ST_String XML_Parser::getAttributeValue(const xercesc::Attributes &attributes,
                                        std::string_view name) {
  if (const auto *value =
          attributes.getValue(XML_xerces_String{name}.getXMLForm())) {
...
std::optional<long>
XML_Parser::getAttributeLongOptional(const xercesc::Attributes &attributes,
...
long XML_Parser::getAttributeLong(const xercesc::Attributes &attributes,
                                  std::string_view name, bool &exists) {
  if (const auto *xmlValue =
          attributes.getValue(XML_xerces_String{name}.getXMLForm())) {
    std::istringstream iss(XML_xerces_String{xmlValue}.getLocalForm().data());
    long result{};
    iss >> result;
    if (!iss.fail()) { // should fail for invalid input, so it shouldn't fail if
                       // attribute long exists
      // iss >> std::ws; // no trailing whitespaces to preserve the original
      // behavior
      if (iss.eof()) { // should fail for unconsumed string and there shouldn't
                       // be any
        exists = true;
        return result;
      }
...
bool XML_Parser::getAttributeBool(const xercesc::Attributes &attributes,
                                  std::string_view name, bool &exists) {
  if (const auto *xmlValue =
          attributes.getValue(XML_xerces_String{name}.getXMLForm())) {
    ...
    static constexpr std::array<XMLCh, 6> false_{
        {'f', 'a', 'l', 's', 'e', '\0'}};
...
std::optional<bool>
XML_Parser::getAttributeBool(const xercesc::Attributes &attributes,
                             std::string_view name) {
  const auto *xmlValue =
      attributes.getValue(XML_xerces_String{name}.getXMLForm());
