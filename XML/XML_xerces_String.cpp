...
XML_xerces_String::XML_xerces_String(std::string_view localForm) {
  ...
  m_LocalForm =
      M::Memory::duplicateUniqueArray(localForm.data(), localForm.size());
}
...
void XML_xerces_String::setLocalForm(std::string_view localForm) {
  ...
  m_XMLForm.reset();
  m_LocalForm =
      M::Memory::duplicateUniqueArray(localForm.data(), localForm.length());
}
...
char *XML_xerces_String::convertToLocalForm(const XMLCh *XMLForm) const {
  ...
  if (m_Transcoder) {
    ...
    const XMLSize_t ResultLength = m_Transcoder->transcodeTo(
        XMLForm, CharsAvailable, reinterpret_cast<unsigned char *>(LocalForm),
        CharsAvailable * 4, CharsEaten, xercesc::XMLTranscoder::UnRep_RepChar);
...
std::string_view XML_xerces_String::getLocalForm() {
  if (!m_LocalForm && m_XMLForm) {
    m_LocalForm =
        M::Memory::as_unique_array_ptr(convertToLocalForm(m_XMLForm.get()));
  }
...
int XML_xerces_String::compareNoCase(const XMLCh *XMLForm) {
  if (const XMLCh *self = getXMLForm();
      !self) // no current XML form and can't generate it
    return XMLForm ? -1 : 0;
