...
public:
  ...
  ConstantReadStream(const TY_Blob *buffer)
      : m_content(buffer ? buffer : &m_placeholder), m_readPosition(0) {}
  ...
  T_uint64 getSize() const override {
    return m_content ? m_content->getSize() : 0;
  }
...
public:
  ...
  /** Write a null terminated string into the stream. The null will NOT be
   * written into the stream, only the string content.
   * @param String The string to add to the stream.
   */
  void write(const char *String);
  /** Write a null terminated string into the stream. The null will NOT be
   * written into the stream, only the string content. The string will not be
   * copied and the stream takes over the ownership about it. The string HAS to
   * be allocated via M_String.
   * @param String The string to add to the stream.
   */
  ...
  /** Terminate the content with 0. This is necessary when M_MemoryStream is
  ...
  /** Get ownership of internal content. The memory stream will be empty but
   * still usable afterwards. Make sure to remove the content with
   * M_Memory::release().
   * @param Content Store pointer to buffer here.
   * @param Size Store size of buffer here.
   */
  ...
  /** Get ownership of internal content. The memory stream will be empty but
   * still usable afterwards. Make sure to remove the content on your own. */
...
private:
  ...
  M_MemoryStreamFragment &m_lastUnflushed() const {
    return m_UnflushedContent.back();
  }
  void m_setReadPosition(T_uint64 newPos) override;
};
