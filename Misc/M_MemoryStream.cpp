...
bool IReadStream::setReadPosition(T_int64 offset, int origin) {
  ...
  T_uint64 positiveOffset =
      static_cast<T_uint64>(isNegativeOffset ? -1 * offset : offset);
  if (origin == SEEK_SET) {
    if (isNegativeOffset) {
      return false;
    }
...
M_MemoryStreamFragment::M_MemoryStreamFragment(char *Content, T_uint64 Size)
    : m_Data(Content), m_UsedSize(Size), m_FreeSize(0) {}

M_MemoryStreamFragment::M_MemoryStreamFragment(const char *Content,
                                               T_uint64 Size)
    : m_Data(nullptr), m_UsedSize(0), m_FreeSize(0) {
...
M_MemoryStreamFragment::M_MemoryStreamFragment(
    M_MemoryStreamFragment &&src) noexcept
    : m_Data(std::move(src.m_Data)), m_UsedSize(src.m_UsedSize),
...
void M_MemoryStreamFragment::append(const char *Content, T_uint64 Size) {
  if (!m_Data) {
    ...
    m_Data = M::Memory::reAllocateUniqueArray<char>(std::move(m_Data),
                                                    m_UsedSize + Size);
    // m_Data = static_cast< char*>( M::Memory::reAllocate( m_Data, m_UsedSize +
    // Size));
...
T_uint64 M_MemoryStreamFragment::getSize() const { return m_UsedSize; }

T_uint64 M_MemoryStreamFragment::getFreeSize() const { return m_FreeSize; }
...
M_MemoryStream::M_MemoryStream() : m_ReadPosition(0), m_Size(0) {}

M_MemoryStream::M_MemoryStream(M_MemoryStream &&src) noexcept
    : m_UnflushedContent(std::move(src.m_UnflushedContent)),
      m_Content(std::move(src.m_Content)), m_ReadPosition(src.m_ReadPosition),
      m_Size(src.m_Size) {
...
M_MemoryStream::M_MemoryStream(const char *String)
    : m_Content(String), m_ReadPosition(0), m_Size(m_Content.getSize()) {}

M_MemoryStream::M_MemoryStream(const TY_Blob *Content)
    : m_Content(Content), m_ReadPosition(0), m_Size(m_Content.getSize()) {}
...
void M_MemoryStream::flush() const {
  ...
  const auto size = std::accumulate(
      m_UnflushedContent.begin(), m_UnflushedContent.end(), m_Content.getSize(),
      [](const T_uint64 sum, const M_MemoryStreamFragment &f) {
...
void M_MemoryStream::write_legacy(long Content) {
  ...
  if (size <= 0)
    return;
  // just in case size > 20 (should not happen since 21 is large enough for
  // long)
  T_uint64 sizeToWrite =
      static_cast<T_uint64>(std::min<int>(size, sizeof(buffer) - 1));
...
void M_MemoryStream::write(const std::string &String) { write(String.c_str()); }

void M_MemoryStream::write(const ST_String &String) { write(String.c_str()); }
...
void M_MemoryStream::write(const char *Content, T_uint64 Size) {
  ...
  // we can theoretically fill in one block if it has the exact free size as the
  // content
...
TY_Blob M_MemoryStream::detachContent() {
  ...
  // so calling reset here is unnecessary here because the move constructor is
  // already clean.
...
T_uint64 M_MemoryStream::getSize() const { return m_Size; }
...
T_uint64 M_MemoryStream::getReadPosition() const { return m_ReadPosition; }
...
void M::ConstantReadStream::getContent(const char **content,
                                       T_uint64 *size) const {
  if (m_content) {
    m_content->getContent(content, size);
  } else {
