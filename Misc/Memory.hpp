 ...
 * @tparam ElementType  The type of element we are allocating memory for. This
 *is also the resulting pointer type.
 *
 * @example M_Memory::allocate<int>( 2) allocates continuous memory fragment for
 *2 integers.
 * @returns A pointer to allocated memory
 */
template <RawAllocatable ElementType> ElementType *allocate(size_t Count) {
  // Size * sizeof(ElementType) can overflow size_t
  if (Count > std::numeric_limits<size_t>::max() / sizeof(ElementType)) {
    outOfMemoryHandler();
  }
  // allocate template should only be used for trivial types since the object is
  // not constructed. so if Foo* p is returned, it's dangerous because
  // p->m_func()
  return static_cast<ElementType *>(allocate(Count * sizeof(ElementType)));
}

/**
 * Allocates memory for some arbitrary number of elements and initialize memory
 *with zero.
 *
 * @param Count Number of elements we are allocating memory for
 * @tparam ElementType  The type of element we are allocating memory for. This
 ...
 * @returns A pointer to allocated memory
 */
template <RawAllocatable ElementType> ElementType *callocate(size_t Count) {
  // Size * sizeof(ElementType) can overflow size_t
  if (Count > std::numeric_limits<size_t>::max() / sizeof(ElementType)) {
    outOfMemoryHandler();
  }
...
struct Releaser {
  void operator()(void *p) const { release(p); }
};

template <class T> using unique_ptr = std::unique_ptr<T, Releaser>;

template <class T> unique_ptr<T> as_unique_ptr(T *ptr) {
  return unique_ptr<T>{ptr};
}

template <class T> unique_ptr<T[]> as_unique_array_ptr(T *ptr) {
  return unique_ptr<T[]>{ptr};
}
...
/** Copy memory area.
 * @param Buffer The buffer to copy. Can have zero bytes which will all be
 * copied up to the Size.
 * @param Size The size of the buffer.
 */
...
template <class T> unique_ptr<T> allocateUnique() {
  return as_unique_ptr(static_cast<T *>(allocate(sizeof(T))));
}

template <class T> unique_ptr<T[]> allocateUniqueArray(size_t count) {
  return as_unique_array_ptr(static_cast<T *>(allocate(sizeof(T) * count)));
}

template <class T> unique_ptr<T[]> callocateUniqueArray(size_t count) {
...
unique_ptr<T[]> duplicateUniqueArray(const T *src, size_t count) {
  return as_unique_array_ptr(
      static_cast<T *>(duplicate(src, sizeof(T) * count)));
}

inline unique_ptr<char[]> duplicateUniqueString(const char *buffer,
                                                size_t size) {
