...
TEST_CASE("M_MemoryStreamFragment: Copy constructor allocates and copies",
          "[misc][streamFragment]") {
  ...
  if (src.size() < FRAGMENT_SIZE) {
    REQUIRE(fragment.getFreeSize() ==
            (FRAGMENT_SIZE * 2u) - static_cast<T_uint64>(src.size()));
  }
}

TEST_CASE("M_MemoryStreamFragment: normal constructor does not copy",
          "[misc][streamFragment]") {
...
TEST_CASE("M_MemoryStreamFragment: move constructor",
          "[misc][streamFragment]") {
  ...
  REQUIRE(ptr_b ==
          non_const_chars); // should be the same pointer as the original
...
TEST_CASE("M_MemoryStreamFragment: move assignment operator",
          "[misc][streamFragment]") {
  ...
  REQUIRE(fragment2.getContent(&ptr2) ==
          0); // the original code would fail here!
  REQUIRE(ptr2 == nullptr);
}

TEST_CASE("M_MemoryStreamFragment: append exact free size",
          "[misc][streamFragment]") {
...
TEST_CASE("M_MemoryStream::write(const char* Content, T_uint64 Size) reuses "
          "last unflushed fragment",
          "[misc][memoryStream]") {
  ...
  // In the original implementation, a new fragment is created even if the old
  // fragment can store the new content
  REQUIRE(stream.m_UnflushedContent.size() == 1);
  ...
  REQUIRE(std::string_view{res_ptr + first_size, free_size} ==
          std::string_view{str_b});
}

TEST_CASE("M_MemoryStream::writeConsume(char* Content, T_uint64 Size) reuses "
          "last fragment",
          "[misc][memoryStream]") {
  M_MemoryStream stream;
  ...
  // Second write: try to exactly fill the free space in the first fragment，and
  // release b
  ...
  REQUIRE(std::string_view{res_ptr, first_size} == std::string_view{str_a});
  REQUIRE(std::string_view{res_ptr + first_size, free_size} ==
          std::string_view{std::string(free_size, 'N')});
}
