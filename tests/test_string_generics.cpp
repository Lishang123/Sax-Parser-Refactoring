...
static_assert(
    StringTypeLegacy<const std::nullptr_t>); // original implementation
static_assert(
    !StringType<const std::nullptr_t>); // reject cv-qualified nullptr_t
...
static_assert(
    StringTypeLegacy<const std::nullptr_t &>); // original implementation
static_assert(
    !StringType<const std::nullptr_t &>); // reject cv/ref-qualified nullptr_t

TEST_CASE("sv(const char*) keeps nullptr distinct from empty-string",
          "[string][generic]") {
