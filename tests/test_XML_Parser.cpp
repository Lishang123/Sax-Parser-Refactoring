...
TEST_CASE("XML_Parser ignores (wrong) XML declaration", "[xml][parser]") {
   ...
   * Since the old condition in 'm_RemoveXMLDeclaration' for advancing the
   * buffer is wrong, this header cannot be removed completely and will lead to
   * error. After fixing the problem, this header can be successfully ignored.
   */
  const char *xml =
  ...
  try {
    ...
    CHECK(std::string(msg.getCode()) ==
          "XMLLM_XR_REPOSITORYPARSER_NATIVE_FATAL_PARSE_ERROR");
    CHECK(std::string(msg.getDescription())
              .find("invalid XML encoding declaration 'UTF-8?'") !=
          std::string::npos);
  }
}

template <typename Parser> static Parser makeParser() { return Parser{}; }
...
public:
  ...
  bool startElementChar(const char * /*uri*/, const char * /*localName*/,
                        const char *qName,
...
TEMPLATE_TEST_CASE("XML_Parser accepts valid attribute value",
                   "[xml][parser][attr][long]", AttrTestParserLegacy,
                   AttrTestParserRefactored) {
  ensure_xerces();

  const char *xml = R"(<Functions count="1000"></Functions>)";
...
TEMPLATE_TEST_CASE("XML_Parser ignores no attribute",
                   "[xml][parser][attr][long]", AttrTestParserLegacy,
                   AttrTestParserRefactored) {
  ensure_xerces();

  const char *xml = R"(<Functions></Functions>)";
...
TEMPLATE_TEST_CASE("XML_Parser rejects invalid attribute value",
                   "[xml][parser][attr][long]", AttrTestParserLegacy,
                   AttrTestParserRefactored) {
  ensure_xerces();

  const char *xml = R"(<Functions count="abc"></Functions>)";
...
TEMPLATE_TEST_CASE("XML_Parser ignores empty attribute value",
                   "[xml][parser][attr][long]", AttrTestParserLegacy,
                   AttrTestParserRefactored) {
  ensure_xerces();

  const char *xml = R"(<Functions count=""></Functions>)";
...
TEMPLATE_TEST_CASE("XML_Parser parses boolean attribute",
                   "[xml][parser][attr][bool]", AttrTestParserLegacy,
                   AttrTestParserRefactored) {
  ensure_xerces();

  const char *xml = R"(<Functions optional="true"></Functions>)";
...
TEMPLATE_TEST_CASE("XML_Parser parses empty (boolean) attribute",
                   "[xml][parser][attr][bool]", AttrTestParserLegacy,
                   AttrTestParserRefactored) {
  ensure_xerces();

  const char *xml = R"(<Functions optional=""></Functions>)";
...
TEMPLATE_TEST_CASE("XML_Parser parses invalid boolean attribute",
                   "[xml][parser][attr][bool]", AttrTestParserLegacy,
                   AttrTestParserRefactored) {
  ensure_xerces();

  const char *xml = R"(<Functions optional="123"></Functions>)";
...
TEST_CASE("XML_Parser parses text attribute", "[xml][parser][attr][value]") {
  ...
  const char *xml = R"(<Functions note="description"></Functions>)";
...
TEST_CASE("XML_Parser parses empty text attribute",
          "[xml][parser][attr][value]") {
  ensure_xerces();

  const char *xml = R"(<Functions note=""></Functions>)";
...
TEST_CASE("XML_Parser parses non-existing attribute",
          "[xml][parser][attr][value]") {
  ensure_xerces();
