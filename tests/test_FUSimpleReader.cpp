...
TEST_CASE("FUSimpleReader parses tabfiles/simple.tab", "[repository][simple]") {
  ...
  auto reader = GENERATE(&functions::repository::simple::readRepo_legacy,
                         &functions::repository::simple::readRepo);

  const fs::path simple_path =
      fs::path(PROJECT_SOURCE_DIR) / "tabfiles" / "simple.tab";
  ...
  CHECK(std::string(functions[2].id.c_str()) ==
        "stringLength...RIT_f2128203875h-1761480648.6_1");
  CHECK(std::string(functions[2].source.c_str()) == "1.1");

  CHECK(std::string(functions[3].id.c_str()) ==
        "stringLength...f2128203875h-1761480648.7_1");
...
TEST_CASE("FUSimpleReader rejects duplicated function IDs",
          "[repository][simple]") {
  ensure_xerces();
...
TEST_CASE("FUSimpleReader rejects unexpected element while expecting Functions",
          "[repository][simple]") {
  ensure_xerces();
...
TEST_CASE("FUSimpleReader rejects unexpected elements while expecting Function",
          "[repository][simple]") {
  ensure_xerces();
...
TEST_CASE(
    "FUSimpleReader rejects unexpected elements while expecting ID/Source",
    "[repository][simple]") {
  ensure_xerces();
  ...
  try {
    ...
    CHECK(std::string(msg.getCode()) == "lm::unexpected_element");
    CHECK(std::string(msg.getDescription())
              .starts_with("Expected one of ['ID', 'Source']"));
  }
}

TEST_CASE("FUSimpleReader rejects unexpected child of ID/Source",
          "[repository][simple]") {
  ensure_xerces();
...
TEST_CASE("FUSimpleReader parses simple doc without <source>",
          "[repository][simple]") {
  ensure_xerces();
...
TEST_CASE("FUSimpleReader parses multiple IDs inside a Function",
          "[repository][simple]") {
  ensure_xerces();
...
TEST_CASE("FUSimpleReader parses simple doc without function child",
          "[repository][simple]") {
  ensure_xerces();
...
TEST_CASE("FUSimpleReader preserves whitespaces", "[repository][simple]") {
  ...
  // Most parsers do trim the whitespaces. If this is the intended behavior is
  // unknown.
  REQUIRE(functions.size() == 1);
...
TEST_CASE("FUSimpleReader validates closing tags", "[repository][simple]") {
  ...
  try {
    ...
    CHECK(std::string(msg.getDescription()).find("expected end of tag 'ID'") !=
          std::string::npos);
  }
}
