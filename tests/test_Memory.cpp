...
TEST_CASE("Memory: reAllocate preserves prefix when growing",
          "[Misc][Memory]") {
...
TEST_CASE("Memory: outOfMemoryHandler throws std::bad_alloc",
          "[Misc][Memory]") {
  REQUIRE_THROWS_AS(M::Memory::outOfMemoryHandler(), std::bad_alloc);
}

TEST_CASE("Memory: duplicate(const char*, size_t) returns NUL terminated",
          "[Misc][Memory]") {
...
TEST_CASE("Memory: duplicate(const void*, size_t) performs deep copy",
          "[Misc][Memory]") {
