...
template <class Spec>
concept FUSpec =
    requires(std::string_view tagName, const char *text,
             typename Spec::Element element,
             typename Spec::Element parentElement, typename Spec::Result &out,
             typename Spec::State &state, std::string_view repo) {
      typename Spec::Element;
      typename Spec::Result;
      typename Spec::State;

      requires std::is_enum_v<typename Spec::Element>; // Spec::Element should
                                                       // be an enum type
      ...
      {
        Spec::allowedChildren(parentElement)
      } -> std::same_as<std::span<const typename Spec::Element>>;
