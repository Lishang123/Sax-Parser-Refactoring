...
template <FUSpec Spec> class FUSReader : public XML_Parser {
public:
  ...
  using State =
      Spec::State; // extensible, current index solution is buggy for sax parser
...
private:
  ...
  bool startElementChar(const char *, const char *localName, const char *,
    ...
    if (std::ranges::find(allowedChildren, current_elem) ==
        allowedChildren.end()) {
      std::vector<std::string_view> allowedChildrenView;
      allowedChildrenView.resize(allowedChildren.size());
      std::ranges::transform(allowedChildren, allowedChildrenView.begin(),
                             Spec::elementName);
      utils::unexpectedElement(localName, allowedChildrenView,
                               m_repository.view());
    }
    return true;
  }
