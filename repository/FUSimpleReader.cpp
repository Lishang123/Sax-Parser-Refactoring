...
private:
  ...
  bool startElementChar(const char *, const char *localName, const char *,
                        const xercesc::Attributes &) override;
  bool endElementChar(const char *, const char *, const char *) override;
  bool charactersChar(const char *, const char *, const char *,
                      const char *text, unsigned int) override;
};
...
Reader::Element Reader::toElement(const std::string_view name) {
  ...
  // const auto it = std::ranges::find(lookup, lowerCaseName,
  // &std::pair<std::string_view, Element>::first);
  const auto it = std::ranges::find_if(lookup, [&lowerCaseName](const auto &x) {
    return lowerCaseName == x.first;
  });
 ...
 */
Functions Reader::read(const TY_Blob &data,
                       const std::string_view repo) && // ref-value qualifier
{
  // stores repo label for diagnostics
  m_repository.set(repo);
  // add these lines to prevent deliberate reuse of this function like:
  // std::move(reader).read() m_functions.clear(); m_elementStack.clear();
 ...
 * @return true
 */
bool Reader::startElementChar(const char *, const char *localName, const char *,
  ...
  case Element::document: {
    if (currentElement != Element::functions) {
      utils::unexpectedElement(localName, functionsArrayElement,
                               m_repository.c_str());
    }
    break;
  }
  case Element::functions: {
    if (currentElement != Element::function) {
      utils::unexpectedElement(localName, functionElement,
                               m_repository.c_str());
    }
    break;
  }
  case Element::function: {
    if (currentElement != Element::id && currentElement != Element::source) {
      utils::unexpectedElement(localName, {IDElement, sourceElement},
                               m_repository.c_str());
    }
    break;
  }
 ...
 * <br>
 * @param text the text content. An empty string will be given if the content is
 * empty.
 ...
 */
bool Reader::charactersChar(const char *, const char *, const char *,
                            const char *text, const unsigned int) {
  if (const auto currentElement = m_elementStack.back();
      currentElement == Element::id) {
    // Checks duplicate IDs against all previously read functions.
    // Made 'function' a const reference.
    // text can never be nullptr, same as function.id.c_str() since it's copied
    // from char*, so this comp is null-safe.
    if (const auto it =
            std::ranges::find_if(m_functions,
                                 [text](const Function &function) {
                                   return !strcmp(text, function.id.c_str());
                                 });
        it != m_functions.end()) {
      utils::fatal("lm::duplicated_function_id",
                   fmt::format("Function with ID '{}' duplicated in {}.", text,
                               m_repository.c_str()));
    }
    ...
    if (!m_currentFuncIndex) {
      utils::fatal("lm::source_before_id",
                   fmt::format("Found <Source> before <ID> while parsing {}.",
                               m_repository.c_str()));
    }
