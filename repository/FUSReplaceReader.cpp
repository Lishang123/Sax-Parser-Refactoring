...
private:
  ...
  bool charactersChar(const char *, const char *, const char *,
                      const char *chars, unsigned int) override;
...
#define X(x)                                                                   \
  ...
  constexpr std::array<std::pair<std::string_view, Element>,
                       static_cast<size_t>(Element::unknown) - 1>
      lookup = {{X(functions), X(function), X(id), X(source), X(pattern),
                 X(replacement)}};
#undef X
  ...
  if (currentElement == Element::id) {
    ...
    if (!m_currentFunction) {
      utils::fatal("lm::other_function_child_before_id",
                   fmt::format("Found <{}> before <ID> while parsing {}.",
                               toString(currentElement), m_repository.c_str()));
    }
