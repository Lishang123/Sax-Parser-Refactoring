...
void unexpectedElement(std::string_view got,
                       const std::vector<std::string_view> &wanted_list,
  ...
  unexpected(fmt::format("Expected {} while parsing {}, got '{}'.", expected,
                         repo, got));
}

void unexpectedElement(std::string_view got, std::string_view wanted,
                       std::string_view repo) {
  unexpected(fmt::format("Expected '{}' while parsing {}, got '{}'.", wanted,
                         repo, got));
}
...
void unexpectedChild(std::string_view got, std::string_view repo) {
  unexpected(
      "lm::unexpected_child_element",
      fmt::format("Found unexpected child element '{}' while parsing {}.", got,
                  repo));
}
