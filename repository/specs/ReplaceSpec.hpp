...
struct ReplaceSpec {
  ...
  static bool isKnown(const Element element) {
  ...
  static std::span<const Element> allowedChildren(const Element parent) {
    ...
    static constexpr Element func[] = {Element::id, Element::source,
                                       Element::pattern, Element::replacement};
    switch (parent) {
  ...
  static void onText(const Element element, const char *text, Result &out,
                     State &state, std::string_view repo) {
    using Function = functions::repository::replace::Function;

    if (element == Element::id) {
      // duplicate check
      if (auto it = std::ranges::find_if(out,
                                         [text](const Function &f) {
                                           return !std::strcmp(text,
                                                               f.id.c_str());
                                         });
          it != out.end()) {
        utils::fatal(
            "lm::duplicated_function_id",
