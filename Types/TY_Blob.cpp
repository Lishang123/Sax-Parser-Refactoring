...
TY_Blob::TY_Blob() : m_size(0), m_content(nullptr) {}

TY_Blob::TY_Blob(const TY_Blob &content) : TY_Blob() { setContent(&content); }

TY_Blob::TY_Blob(const TY_Blob *content) : TY_Blob() { setContent(content); }

TY_Blob::TY_Blob(TY_Blob &&content) noexcept
    : m_size(content.m_size), m_content(content.m_content) {
  content.m_content = nullptr;
  content.m_size = 0;
}

TY_Blob::TY_Blob(const char *content) : TY_Blob() { setContent(content); }

TY_Blob::TY_Blob(const void *content, T_uint64 size) : TY_Blob() {
  setContent(content, size);
}

TY_Blob::TY_Blob(char *content, bool consume) : TY_Blob() {
  if (content) {
    setContent(content, strlen(content), consume);
  }
}

TY_Blob::TY_Blob(void *content, T_uint64 size, bool consume) : TY_Blob() {
  setContent(static_cast<char *>(content), size, consume);
}

TY_Blob::~TY_Blob() { reset(); }
...
void TY_Blob::release() { delete this; }
...
T_uint64 TY_Blob::copyContent(char *buffer, T_uint64 offset,
                              T_uint64 size) const {
  if (!m_content || !buffer)
    return 0;
...
T_uint64 TY_Blob::getSize() const { return m_size; }
