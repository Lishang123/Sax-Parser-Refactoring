...
template <class C> struct release_deleter {
...
public:
  ...
  // because of the deleter, never use this pointer with TY_Blob created on the
  // stack!!
  ...
  template <class... Types> static TY_Blob *create(Types &&...args) {
  ...
  /** Get ownership of internal buffer. The blob will be empty but still usable
   * afterwards. Make sure to remove the buffer with M::Memory::release().
   * @param content Store pointer to content here. May be NULL.
   * @param size Store pointer to size of content here. May be NULL.
   */
  ...
  /** Set new content. This makes a copy of the new content. The terminating 0
   * will not be copied.
  ...
  /** Set new content and size. This makes a copy of the new content if Consume
   * is false. Otherwise the ownership changes to this object. In that case,
   * Content has to be allocated via M::Memory.
   * @param content New content.
   * @param size Size of new content.
